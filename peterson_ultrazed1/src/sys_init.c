...
static int platform_register_metal_device(void)
{
	...
	for (i = 0; i < sizeof(metal_dev_table)/sizeof(struct metal_device); i++) {
		...
		//LPRINTF("Core1: registering: %d, name=%s\r\n", i, dev->name);
...
static int open_metal_devices(void)
{
	...
	if (status) {
		//LPERROR("Core1: Failed to open device %s.\r\n", SHM_DEV_NAME);
		return status;
	}
...
int sys_init()
{
	...
	if (status) {
		//LPERROR("Core1: %s: failed to register devices: %d\r\n", __func__, status);
		return status;
	}
	...
	if (status) {
		//LPERROR("Core1: %s: failed to open devices: %d\r\n", __func__, status);
		return status;
	}
...
//	metal_io_write32(shm_io, SHM_DEMO_CNTRL_OFFSET, DEMO_STATUS_IDLE);
	...
	//LPRINTF("Core1: Writting in memory.\r\n");
	...
	//LPRINTF("Core1: Writting done.\r\n");

	//LPRINTF("Core1: Shared memory test finished\r\n");
...
int shared_memory_run()
{
	...
	//LPRINTF("Core1: Shared memory run \r\n");
	...
	if (!io) {
		//LPERROR("Core1: Failed to map io region for %s.\n", shm_dev->name);
		status = -ENODEV;
		return status;
	}
